...
void printMan()
{
    ...
    write(1, "# Para imprimir todas as vendas:    v                          #\n", 65);
    write(1, "################################################################\n\n", 65);
}
...
void printVendasMA(){
    int fd_vendas = open(VENDAS, O_RDONLY, 0777);
    struct venda *vnd = malloc(sizeof(struct venda));
    char *buf = malloc(80);
    int i = 1;

    while(read(fd_vendas, vnd, sizeof(struct venda)) > 0) {
        sprintf(buf,"Venda %d || idArtigo: %d || Quantidade: %d || Total: %f\n", i, vnd->idArtigo, vnd->quantidade, vnd->total);
